...
- **每个账号绑定一个固定出口节点** — agentrouter 对同一 IP 的连续请求会限流（返回 WAF 页而非 JSON），多账号时排在后面的会稳定失败。配置了 mihomo 代理时，每个走代理的账号会拿到**自己专属的一个出口节点**（通过 Clash API，`CHECKIN_PROXY_CONTROLLER`）：各地区轮流填队列、账号按名字排序入座，所以前几个账号自然落在不同国家，且**任何两个账号都不会共用一个 IP**。注意不是「每次换一个节点」——同一个账号的出口 IP 天天变动，本身就是平台判定账号异常的特征，所以只有节点探测不通时才顺延，而且**先在同一个国家里换，整个国家都不通才跨国**。在 `accounts.json` 里调整账号顺序不会让任何账号换国家。地区池限定在港台新日韩（欧美/南美节点实测会 `Page.goto timeout`），订阅里「剩余流量：xx GB」这类信息占位节点和「5倍消耗」这类高倍率节点都排除在外。读取失败时仍冷却 20 秒重试（`CHECKIN_AUTO_RETRY_ATTEMPTS` / `CHECKIN_AUTO_RETRY_DELAY_S` 可调）
...
`CHECKIN_PROXY_CONTROLLER` 指向 mihomo 的 Clash API（默认 `http://127.0.0.1:9091`，端口可用 `PROXY_CONTROLLER_PORT` 改）。设置后，每个走代理的账号会绑定订阅里的**一个固定出口节点**——各地区轮流填队列、账号按名字排序入座，所以账号之间互不共用 IP，而单个账号每天又落在同一个节点上（IP 稳定比 IP 分散更重要，同一个号频繁换出口本身就可疑）。节点不通时先在同地区顺延，整个地区都不通才跨地区，全部探测失败则回退到自动选优（`AUTO` 组）。不设置则所有账号共用同一出口。
